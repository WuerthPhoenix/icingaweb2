...
class Csp
{
    ...
    {
        return array_merge(
            self::fetchNavigationItems('menu-item'),
            self::fetchNavigationItems('dashboard-pane'),
            self::fetchDashletsItems()
        );
    }

    /**
     * Fetches navigation items of a specific type that have an external URL
     * and do not open in a new tab.
     *
     * @param string $type The type of navigation menu to load.
     * @return array A list of navigation items with their names and absolute URLs.
     * // returns [['name' => 'Menu Item 1', 'url' => 'https://external.link'], ...]
     */
    protected static function fetchNavigationItems(string $type)
    {
        $navigation = new Navigation();
        $navigation->load($type);
        $menuItems = [];
        foreach ($navigation->getItems() as $item) {
            $url = $item->getUrl();
            if ($url->isExternal() && $item->getTarget() !== "_blank") {
